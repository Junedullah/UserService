...
public class ControllerUser {
	...
	public ResponseMessage createUser(HttpServletRequest request, @RequestBody DtoUser dtoUser) {
		 ...
		 */
		// User userNameCheck =
		// repositoryUser.findByusernameAndIsDeleted(dtoUser.getEmail(), false);
		/*
...
//		RequestResponseLogger.logResponse(dtoRequestResponseLog, responseMessage);
	 ...
	 * @return
	 *//*
		 * @RequestMapping(value = "/deleteMutipleUsers", method = RequestMethod.POST,
		 * produces = "application/json") public ResponseMessage
		 * deleteMutipleUsers(HttpServletRequest request, @RequestBody DtoUser dtoUser)
		 * { ResponseMessage responseMessage = null; UserSession session =
		 * sessionManager.validateUserSessionId(request); if (session == null) {
		 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
		 * HttpStatus.UNAUTHORIZED,
		 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
		 * false)); return responseMessage; } else { if (dtoUser.getUserIds() != null &&
		 * !dtoUser.getUserIds().isEmpty()) {
		 * serviceUser.deleteMultiPleUsers(dtoUser.getUserIds()); responseMessage = new
		 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
		 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
		 * USER_DELETED_SUCCESS, false)); } else { responseMessage = new
		 * ResponseMessage(HttpStatus.BAD_REQUEST.value(), HttpStatus.BAD_REQUEST,
		 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.LIST_IS_EMPTY,
	 ...
	 */
	/*
	 * @RequestMapping(value = "/getUsersList", method = RequestMethod.PUT, produces
	 * = "application/json") public ResponseMessage getUsersList(HttpServletRequest
	 * request, @RequestBody DtoSearch dtoSearch) { DtoRequestResponseLog
	 * dtoRequestResponseLog = RequestResponseLogger.logRequest(request, dtoSearch);
	 * ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 * HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); return responseMessage; } else { dtoSearch =
	 * serviceUser.getUsersList(dtoSearch); if (dtoSearch.getRecords() != null) {
	 * 
	 * @SuppressWarnings("unchecked") List<DtoUser> list = (List<DtoUser>)
	 * dtoSearch.getRecords(); if (list!=null && !list.isEmpty()) { responseMessage
	 * = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.USER_SUCCESS,
	 * false), dtoSearch); } else { responseMessage = new
	 * ResponseMessage(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.RECORD_NOT_FOUND,
	 * false));
	 * 
	 * } } } RequestResponseLogger.logResponse(dtoRequestResponseLog,
	 ...
	 */
	/*
	 * @RequestMapping(value = "/logoutBeforeCompanySelection", method =
	 * RequestMethod.POST)
	 * 
	 * @ResponseBody public ModelMap
	 * logoutServiceBeforeCompanySelection(@RequestBody DtoUser user,
	 * HttpServletRequest request) { LOGGER.info(" user logout service :::::: ");
	 * ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionIdBeforeCompanySelection(request); if
	 * (session == null) { responseMessage = new
	 * ResponseMessage(HttpStatus.UNAUTHORIZED.value(), HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted("SESSION_EXPIRED", false)); }
	 * else { User userExist =
	 * repositoryUser.findByUserIdAndIsDeleted(user.getUserId(), false); if
	 * (userExist != null) { boolean result =
	 * sessionManager.clearSessionLog(userExist.getUserId()); if (result) {
	 * responseMessage = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
	 * USER_LOGOUT_SUCCESSFULLY, false)); } else { responseMessage = new
	 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); } } else { responseMessage = new
	 * ResponseMessage(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.USER_NOT_FOUND,
	 * false)); } } return new ModelMap("response", responseMessage); }
	 * 
	 *//**
		 * @description : Get User Detail By UserId
		 * @param request
	 ...
	 * RequestMethod.POST, produces = "application/json") public ResponseMessage
	 * getUserDetailByUserId(HttpServletRequest request, @RequestBody DtoUser
	 * dtoUser) { ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 * HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); return responseMessage; } else { DtoUser users =
	 * serviceUser.getUsersDetailByUserId(dtoUser); responseMessage = new
	 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
	 * USER_DETAIL_SUCCESS, false), users); } return responseMessage; }
	 */
	 ...
	 *//*
		 ...
		 * produces = "application/json") public ResponseMessage
		 * checkUserName(HttpServletRequest request, @RequestBody DtoUser dtoUser) {
		 * ResponseMessage responseMessage = null; if (!dtoUser.getUserName().isEmpty())
		 * { User users =
		 * repositoryUser.findByusernameAndIsDeleted(dtoUser.getUserName(), false); if
		 * (users != null) { responseMessage = new
		 * ResponseMessage(HttpStatus.FOUND.value(), HttpStatus.FOUND,
		 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
		 * USER_NAME_ALREADY_EXIST, false)); } else { responseMessage = new
		 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
		 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
		 * USER_NAME_NOT_EXIST, false)); } } else { responseMessage = new
		 * ResponseMessage(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND,
		 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.USER_NAME_BLANK,
		 * false)); } return responseMessage; }
		 */
	/**
	...
	/*
	 * @RequestMapping(value = "/saveAuthorizationDetail", method =
	 * RequestMethod.POST)
	 * 
	 * @ResponseBody public ResponseMessage saveAuthorizationDetail(@RequestBody
	 * DtoAuthorizationDetail dtoAuthorization, HttpServletRequest request) throws
	 * ParseException { LOGGER.info(" save Authorization Detail service :::::: ");
	 * ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 * HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); } else { dtoAuthorization =
	 * serviceUser.saveAuthorizationDetail(dtoAuthorization); responseMessage = new
	 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
	 * AUTHORIZATION_DETAIL_SAVED, false), dtoAuthorization); } return
	 * responseMessage; }
	 * 
	 *//**
		 * @description : Check Email
		 ...
		 * @return
		 *//*
			 * @RequestMapping(value = "/checkEmail", method = RequestMethod.POST, produces
			 * = "application/json") public ResponseMessage checkEmail(HttpServletRequest
			 * request, @RequestBody DtoUser dtoUser) { ResponseMessage responseMessage =
			 * null; if (!dtoUser.getEmail().isEmpty()) { List<User> usersList =
			 * repositoryUser.findByIsDeletedAndEmail(false, dtoUser.getEmail()); if
			 * (usersList != null && !usersList.isEmpty()) { responseMessage = new
			 * ResponseMessage(HttpStatus.FOUND.value(), HttpStatus.FOUND,
			 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.USER_EMAIL_EXIST,
			 * false)); } else { responseMessage = new
			 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
			 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
			 * USER_EMAIL_NOT_EXIST, false)); } } else { responseMessage = new
			 * ResponseMessage(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND,
			 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.USER_EMAIL_BLANK,
			 * false)); } return responseMessage; }
			 */
	 ...
	 */
	/*
	 * @RequestMapping(value = "/getUsersListForDropDown", method =
	 * RequestMethod.PUT, produces = "application/json") public ResponseMessage
	 * getUsersListForDropDown(HttpServletRequest request, @RequestBody DtoSearch
	 * dtoSearch) { ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 * HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); return responseMessage; } else { DtoSearch dtoSearch2 =
	 * serviceUser.getUsersListForDropDown(dtoSearch); if (dtoSearch2.getRecords()
	 * != null) {
	 * 
	 * @SuppressWarnings("unchecked") List<DtoUser> list = (List<DtoUser>)
	 * dtoSearch2.getRecords(); if (list != null && !list.isEmpty()) {
	 * responseMessage = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.USER_SUCCESS,
	 * false), dtoSearch2);
	 * 
	 * } else { responseMessage = new ResponseMessage(HttpStatus.NOT_FOUND.value(),
	 ...
	 */
	/*
	 * @RequestMapping(value = "/setUserIP", method = RequestMethod.PUT) public
	 * ResponseMessage setUserIP(HttpServletRequest request, @RequestBody DtoUserIp
	 * dtoUserIp, ResponseMessage responseMessage) { if (dtoUserIp.getIpAddress() !=
	 * null) { UserSession session = sessionManager.validateUserSessionId(request);
	 * if (session == null) { responseMessage = new
	 * ResponseMessage(HttpStatus.UNAUTHORIZED.value(), HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); return responseMessage; } else { DtoUserIp dtoUserIp2 =
	 * serviceUser.setUserIPForAuthentication(dtoUserIp); if
	 * (dtoUserIp2.getMessageType() == null) { responseMessage = new
	 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.IP_ADDED, false),
	 * true); } else { responseMessage = new ResponseMessage(HttpStatus.OK.value(),
	 * HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(dtoUserIp2.getMessageType(),
	 * false), false); } } } return responseMessage; }
	 * 
	 *//**
		 ...
		 */
	/*
	 * @RequestMapping(value = "/updateUserIP", method = RequestMethod.PUT) public
	 * ResponseMessage updateUserIP(HttpServletRequest request, @RequestBody
	 * DtoUserIp dtoUserIp, ResponseMessage responseMessage) { if
	 * (dtoUserIp.getIpAddress() != null) { UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 * HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); return responseMessage; } else { Boolean isIPUpdated =
	 * serviceUser.updateUserIPForAuthentication(dtoUserIp); if (isIPUpdated) {
	 * responseMessage = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.IP_UPDATED,
	 * false), true); } else { responseMessage = new
	 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.IP_REQUIRED,
	 * false), false); } } } return responseMessage; }
	 * 
	 *//**
		 * @description : Search Users
	...
	/*
	 * @RequestMapping(value = "/searchUsers", method = RequestMethod.POST, consumes
	 * = MediaType.APPLICATION_JSON_VALUE, produces =
	 * MediaType.APPLICATION_JSON_VALUE, headers = "Accept=application/json") public
	 * ResponseMessage searchUsers(@RequestBody DtoSearch dtoSearch,
	 * HttpServletRequest request) { DtoRequestResponseLog dtoRequestResponseLog =
	 * RequestResponseLogger.getInstance().logRequest(request, dtoSearch);
	 * ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session != null) {
	 * dtoSearch = this.serviceUser.searchUsers(dtoSearch); if (dtoSearch != null &&
	 * dtoSearch.getRecords() != null) { responseMessage = new
	 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * this.serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.USER_SUCCESS,
	 * false), dtoSearch); } else { responseMessage = new
	 * ResponseMessage(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.RECORD_NOT_FOUND,
	 * false)); }
	 ...
	 * ResponseMessage(HttpStatus.UNAUTHORIZED.value(), HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); } RequestResponseLogger.logResponse(dtoRequestResponseLog,
	 * responseMessage); return responseMessage; }
	 * 
	 *//**
	 ...
	 * @RequestMapping(value = "/logout", method = RequestMethod.POST)
	 * 
	 * @ResponseBody public ModelMap logoutService(@RequestBody DtoUser user,
	 * HttpServletRequest request) { LOGGER.info(" user logout service :::::: ");
	 * ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 ...
	 * responseMessage = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse
	 * .getMessageByShortAndIsDeleted(MessageLabel.USER_LOGOUT_SUCCESSFULLY,
	 * false)); } else { responseMessage = new
	 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); } } else { responseMessage = new
	 ...
	 */
	/*
	 * @RequestMapping(value = "/saveMacAddress", method = RequestMethod.POST,
	 * produces = "application/json") public ResponseMessage
	 * saveMacAddress(HttpServletRequest request, @RequestBody DtoUser dtoUser) {
	 * ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 * HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); return responseMessage; } else { User user =
	 * repositoryUser.findByUserIdAndIsDeleted(dtoUser.getUserId(), false); if (user
	 * != null) { boolean response = serviceUser.saveMacAddress(dtoUser, user); if
	 * (response) { responseMessage = new ResponseMessage(HttpStatus.OK.value(),
	 * HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.MAC_ADDRESS_ADDED,
	 * false)); } else { responseMessage = new
	 * ResponseMessage(HttpStatus.BAD_REQUEST.value(), HttpStatus.BAD_REQUEST,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.BAD_REQUEST,
	 * false)); } } else { responseMessage = new
	 * ResponseMessage(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.USER_NOT_FOUND,
	 * false)); } } return responseMessage; }
	...
	/*
	 * @RequestMapping(value = "/deleteMacAddress", method = RequestMethod.POST,
	 * produces = "application/json") public ResponseMessage
	 * deleteMacAddress(HttpServletRequest request, @RequestBody DtoUser dtoUser) {
	 * ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 * HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); return responseMessage; } else { boolean response =
	 * serviceUser.deleteMacAddressOfUser(dtoUser); if (response) { responseMessage
	 * = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
	 * MAC_ADDRESS_DELETED, false)); } else { responseMessage = new
	 * ResponseMessage(HttpStatus.BAD_REQUEST.value(), HttpStatus.BAD_REQUEST,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.BAD_REQUEST,
	 * false)); } } return responseMessage; }
	 * 
	 ...
	 * produces = "application/json") public ResponseMessage
	 * getAllUsersList(HttpServletRequest request, @RequestBody DtoSearch dtoSearch)
	 * { ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 * HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); return responseMessage; } else { DtoSearch dtoSearch2 =
	 * serviceUser.getAllUsersList(dtoSearch); if (dtoSearch2.getRecords() != null)
	 * {
	 * 
	 * @SuppressWarnings("unchecked") List<DtoUser> list = (List<DtoUser>)
	 * dtoSearch2.getRecords(); if (list != null && !list.isEmpty()) {
	 ...
	 * 
	 * @RequestMapping(value = "/updateIpCheckedStatus", method =
	 * RequestMethod.POST, produces = "application/json") public ResponseMessage
	 * updateIpCheckedStatus(HttpServletRequest request, @RequestBody DtoUser
	 * dtoUser) { ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 * HttpStatus.UNAUTHORIZED,
	 ...
	 * !dtoUser.getUserIds().isEmpty()) {
	 * 
	 * boolean response = serviceUser.updateIpCheckStatusOfUser(dtoUser); if
	 * (response) { responseMessage = new ResponseMessage(HttpStatus.OK.value(),
	 * HttpStatus.OK, serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
	 * IP_CHECKED_STATUS_UPDATED, false)); } else { responseMessage = new
	 * ResponseMessage(HttpStatus.INTERNAL_SERVER_ERROR.value(),
	 * HttpStatus.INTERNAL_SERVER_ERROR,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.ERROR_OCCURED,
	 * false), null); } } else { responseMessage = new
	 * ResponseMessage(HttpStatus.BAD_REQUEST.value(), HttpStatus.BAD_REQUEST,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.LIST_IS_EMPTY,
	 * false)); } return responseMessage; }
	 * 
	 *//**
		 ...
		 *//*
			 ...
			 * 
			 * @ResponseBody public ResponseMessage sideBarMenuDetail(HttpServletRequest
			 * request) { LOGGER.info("Get Screen Detail"); ResponseMessage responseMessage
			 * = null;
			 * 
			 * UserSession session = sessionManager.validateUserSessionId(request); if
			 * (session == null) { responseMessage = new
			 * ResponseMessage(HttpStatus.UNAUTHORIZED.value(), HttpStatus.UNAUTHORIZED,
			 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
			 * false)); return responseMessage; } else { List<DtoHeader> dtoHeaderList =
			 * serviceUser.getSideBarMenuDetail(); if (dtoHeaderList != null) {
			 * responseMessage = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
			 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
			 * HEADER_RECORD_FOUND, false), dtoHeaderList); } else { responseMessage = new
			 * ResponseMessage(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND,
			 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.RECORD_NOT_FOUND,
			 * false)); } } return responseMessage; }
			 * 
			 ...
			 * 
			 * @ResponseBody public ResponseMessage sideMenuByModuleId(HttpServletRequest
			 * request, @RequestBody DtoHeader dtoHeader) {
			 * LOGGER.info("Get Screen Detail"); ResponseMessage responseMessage = null;
			 * 
			 * UserSession session = sessionManager.validateUserSessionId(request); if
			 * (session == null) {
			 * 
			 ...
			 * if (dtoHeaderList != null) { responseMessage = new
			 * ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
			 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.
			 * HEADER_RECORD_FOUND, false), dtoHeaderList); } else { responseMessage = new
			 * ResponseMessage(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND,
	...
	/*
	 * @RequestMapping(value = "/getActiveUsersList", method = RequestMethod.PUT,
	 * produces = "application/json") public ResponseMessage
	 * getActiveUsersList(HttpServletRequest request, @RequestBody DtoSearch
	 * dtoSearch) { ResponseMessage responseMessage = null; UserSession session =
	 * sessionManager.validateUserSessionId(request); if (session == null) {
	 * responseMessage = new ResponseMessage(HttpStatus.UNAUTHORIZED.value(),
	 * HttpStatus.UNAUTHORIZED,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.FORBIDDEN,
	 * false)); return responseMessage; } else { dtoSearch =
	 * serviceUser.getActiveSessionUsersList(dtoSearch, session); if
	 * (dtoSearch.getRecords() != null) {
	 * 
	 * @SuppressWarnings("unchecked") List<DtoUser> list = (List<DtoUser>)
	 * dtoSearch.getRecords(); if (list != null && !list.isEmpty()) {
	 * responseMessage = new ResponseMessage(HttpStatus.OK.value(), HttpStatus.OK,
	 * serviceResponse.getMessageByShortAndIsDeleted(MessageLabel.USER_SUCCESS,
	 * false), dtoSearch); } else { responseMessage = new
	 * ResponseMessage(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND,
	 ...
	 * @RequestMapping(value = "/getAllUserDetail", method = RequestMethod.GET)
	 * 
	 * @ResponseBody public ResponseMessage getAllUserDetail(HttpServletRequest
	 * request) { LOGGER.info("GetAll UserDetail URL called"); ResponseMessage
	 * responseMessage = null; List<DtoUserDetail> userDetailList =
	 * serviceUser.getUserDetailList(); if (userDetailList != null &&
	 * userDetailList.size() > 0) { responseMessage = new
	 * ResponseMessage(HttpStatus.CREATED.value(), HttpStatus.CREATED,
	 * serviceResponse.getMessageByShortAndIsDeleted("USER_DETAILS_SUCCESS", false),
	 * userDetailList);
	 * 
	 * } else { responseMessage = new
	 * ResponseMessage(HttpStatus.BAD_REQUEST.value(), HttpStatus.BAD_REQUEST,
